 ...
 *  - `throttleMs`: valor exibido em Settings (informacional; o service usa um
 *    const hoje, mas fica pronto para torná-lo configurável no futuro).
...
class TruckPrefs(context: Context) {
    ...
    var throttleMs: Long
        get() = prefs.getLong(KEY_THROTTLE_MS, DEFAULT_THROTTLE_MS)
        set(value) = prefs.edit().putLong(KEY_THROTTLE_MS, value).apply()
     ...
     * (item "Limpar dispositivo pareado" em Settings).
     */
    ...
    companion object {
        ...
        const val DEFAULT_THROTTLE_MS = 80L
    }
}
