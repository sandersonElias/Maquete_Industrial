...
export default function MaqueteSection() {
  ...
  const [info, setInfo] = useState({ title: 'Visão Geral', text: VIEWS[0].text });
  ...
  const handleTrainAdded = useCallback((train) => {
    setTrains(prev => [...prev, {
      id: train.id,
      name: train.name,
      color: train.color,
  ...
  const handleTrainRemoved = useCallback((id) => {
    setTrains(prev => prev.filter(t => t.id !== id));
    setTimeout(updateTrainCount, 350);
  }, [updateTrainCount]);

  const handleTrainToggled = useCallback((train) => {
  ...
  const handleAllToggled = useCallback((running) => {
    setTrains(prev => prev.map(t => ({ ...t, running })));
  }, []);

  const handleReversor = useCallback((state) => {
  ...
  const handleSwitch = useCallback((label, state) => {
  ...
  const handleGlobalSpeed = (e) => {
    const speed = parseFloat(e.target.value);
    setGlobalSpeed(speed);
    setGlobalSpeedVal(Math.round((speed / 0.1) * 100) + '%');
    if (sceneRef.current) {
      sceneRef.current.trains.forEach(t => {
        t.targetSpeed = speed;
      });
    }
  };

  const toggleTrainRunning = (id) => {
    sceneRef.current?.toggleTrainRunning(id);
  };

  const reverseTrain = (id) => {
                ...
                <div className="train-list">
                  ...
                  {trains.map((train) => (
                    <TrainCard
                      ...
                      onSpeedChange={(speed) => setTrainSpeed(train.id, speed)}
                    />
                  ))}
                </div>
...
function TrainCard({ train, onToggle, onReverse, onDelete, onSpeedChange }) {
