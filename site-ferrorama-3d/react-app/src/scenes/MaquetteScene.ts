...
interface TrackCurve {
  curve: {
    ...
    getPoints(count: number): THREE.Vector3[];
  };
  name: string;
  elevation: number;
}
...
export default class MaquetteScene {
  ...
  mouse: THREE.Vector2;
  hoverIndicator: THREE.Mesh | null;
  placementMode: boolean;
  selectedTrainType: number;

  reversor: THREE.Group | null;
  reversorRed: THREE.Mesh | null;
  reversorGreen: THREE.Mesh | null;
  reversorState: string;
  ...
  onTrainAdded: ((train: TrainObject) => void) | null;
  onTrainRemoved: ((id: number) => void) | null;
  onTrainToggled: ((train: TrainObject) => void) | null;
  onTrainPlaced: (() => void) | null;
  onAllTrainsToggled: ((running: boolean) => void) | null;
  onReversorToggled: ((state: string) => void) | null;
  onSwitchToggled: ((label: string, state: string) => void) | null;
  ...
  _buildStraightTrack(waypoints: THREE.Vector3[]) {
    var segments: THREE.LineCurve3[] = [];
    ...
    return {
      ...
      getPoints: function (count: number) {
  ...
  _createJunctionMarker(x: number, y: number, z: number) {
  ...
  _renderRail(
    curve: { getPointAt(t: number): THREE.Vector3; getTangentAt(t: number): THREE.Vector3; getPoints(count: number): THREE.Vector3[] },
    ...
    var renderRailSide = (points: THREE.Vector3[]) => {
  ...
  setupClickHandler() {
    ...
    canvas.addEventListener('click', function (e: MouseEvent) {
      ...
      var best: number | null = null, bestD = Infinity;
      for (var i = 0; i < pts.length; i++) {
      ...
      if (best !== null) {
        ...
        (self.hoverIndicator!.material as THREE.MeshBasicMaterial).opacity = 0;
        if (typeof self.onTrainPlaced === 'function') self.onTrainPlaced();
      }
    });

    canvas.addEventListener('mousemove', function (e: MouseEvent) {
      if (!self.placementMode) { (self.hoverIndicator!.material as THREE.MeshBasicMaterial).opacity = 0; return; }
      var rect = canvas.getBoundingClientRect();
      self.mouse.x = ((e.clientX - rect.left) / rect.width) * 2 - 1;
      self.mouse.y = -((e.clientY - rect.top) / rect.height) * 2 + 1;
      self.raycaster.setFromCamera(self.mouse, self.camera!);
      var pts = self.trackCurves[0].curve.getPoints(300);
      var best: THREE.Vector3 | null = null, bestD = Infinity;
      for (var i = 0; i < pts.length; i++) {
        var pt = pts[i].clone(); pt.y = 0.5;
        var pr = new THREE.Vector3();
        self.raycaster.ray.closestPointToPoint(pt, pr);
      ...
      if (best) {
        ...
        (self.hoverIndicator!.material as THREE.MeshBasicMaterial).opacity = bestD < 2.0 ? 0.8 : 0.3;
      } else {
        (self.hoverIndicator!.material as THREE.MeshBasicMaterial).opacity = 0;
      }
    });
  ...
  setPlacementMode(active: boolean, typeIndex: number) {
    this.placementMode = active;
    this.selectedTrainType = typeIndex || 0;
    if (!active) (this.hoverIndicator!.material as THREE.MeshBasicMaterial).opacity = 0;
  }
  ...
  toggleReversor() {
    if (this.reversorState === 'green') {
      ...
      (this.reversorRed!.material as THREE.MeshStandardMaterial).emissiveIntensity = 0.8;
      (this.reversorGreen!.material as THREE.MeshStandardMaterial).emissiveIntensity = 0.1;
    } else {
      this.reversorState = 'green';
      (this.reversorRed!.material as THREE.MeshStandardMaterial).emissiveIntensity = 0.1;
      (this.reversorGreen!.material as THREE.MeshStandardMaterial).emissiveIntensity = 0.8;
    }
  ...
  setupSwitchClick() {
    ...
    this.scene!.traverse(function (c: THREE.Object3D) {
      if (c.userData && c.userData.type === 'switch') {
        self.switches.push(c as THREE.Group);
  ...
  toggleSwitch(sw: THREE.Group) {
    ...
    var lever = sw.children[2] as THREE.Mesh;
    if (lever) lever.rotation.z = next === 'left' ? Math.PI / 4 : -Math.PI / 4;

    var mech = sw.children[1] as THREE.Mesh;
    if (mech && mech.material) (mech.material as THREE.MeshStandardMaterial).emissiveIntensity = next === 'right' ? 0.6 : 0.2;
  ...
  // TRAIN CREATION
  // ==========================================
  _makeWheel(radius: number, width: number) {
    var g = new THREE.Group();
  ...
  _addMesh(group: THREE.Group, geo: THREE.BufferGeometry, mat: THREE.Material, px: number, py: number, pz: number) {
  ...
  addTrain(progressOnTrack: number, colorIndex: number, typeIndex: number) {
  ...
  removeTrain(id: number) {
    ...
    train.group.traverse(function (c: THREE.Object3D) {
      if ((c as THREE.Mesh).geometry) (c as THREE.Mesh).geometry.dispose();
      if ((c as THREE.Mesh).material) {
        if (Array.isArray((c as THREE.Mesh).material)) ((c as THREE.Mesh).material as THREE.Material[]).forEach(function (m) { m.dispose(); });
        else ((c as THREE.Mesh).material as THREE.Material).dispose();
      }
    });
  ...
  setTrainSpeed(id: number, speed: number) {
    for (var i = 0; i < this.trains.length; i++) {
      if (this.trains[i].id === id) { this.trains[i].targetSpeed = speed; return; }
    }
  }

  toggleTrainRunning(id: number) {
    for (var i = 0; i < this.trains.length; i++) {
      var t = this.trains[i];
      if (t.id === id) {
        t.running = !t.running;
  ...
  reverseTrain(id: number) {
    for (var i = 0; i < this.trains.length; i++) {
      if (this.trains[i].id === id) { this.trains[i].direction *= -1; return; }
    }
  }

  getTrainCount(): number { return this.trains.length; }
  ...
  _placeTrainOnTrack(train: TrainObject) {
  ...
  animateToView(view: string) {
    var views: Record<string, ViewConfig> = {
  ...
  animate() {
    ...
    this.scene!.traverse(function (c: THREE.Object3D) {
      ...
      if (c.userData && c.userData.type === 'reversor') {
        c.children.forEach(function (child: THREE.Object3D) {
          if ((child as THREE.Mesh).material && ((child as THREE.Mesh).material as THREE.MeshStandardMaterial).emissive) {
            if (((child as THREE.Mesh).material as THREE.MeshStandardMaterial).emissiveIntensity > 0.5) {
              ((child as THREE.Mesh).material as THREE.MeshStandardMaterial).emissiveIntensity = reversorGlow;
            }
          }
        });
      }
  ...
  onResize() {
    if (!this.container) return;
    var w = this.container.clientWidth || (this.container as HTMLElement).offsetWidth;
    var h = this.container.clientHeight || (this.container as HTMLElement).offsetHeight;
    if (w === 0 || h === 0) return;
